...
def create_model(existing='', is_twohundred=False, is_halffeatures=True):
    ...
    if len(existing) == 0:
        ...
        def upproject(tensor, filters, name, concat_with, shape_tensor, size):
            up_i = BilinearUpSampling2D(input_sh=shape_tensor, size=size, name=name+'_upsampling2d')([shape_tensor,tensor])
        ...
        decoder = upproject(decoder, int(decode_filters/2), 'up1', concat_with='pool3_pool',shape_tensor=sh, size=(2,2))
        decoder = upproject(decoder, int(decode_filters/4), 'up2', concat_with='pool2_pool',shape_tensor=sh, size=(4,4))
        decoder = upproject(decoder, int(decode_filters/8), 'up3', concat_with='pool1',shape_tensor=sh, size=(8,8))
        decoder = upproject(decoder, int(decode_filters/16), 'up4', concat_with='conv1/relu',shape_tensor=sh, size=(16,16))
